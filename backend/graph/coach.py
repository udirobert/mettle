...
Two stages before the multi-perspective debate:
  0. Scenario resolution: a custom scenario described by the user (typed or
     via voice interview, in state.transcript) is synthesized into stakes /
     counterpart persona / opening line; otherwise the lp_renewal default
     loads. The interview is then consumed and the transcript cleared so
     rehearsal starts clean.
  1. Multi-perspective debate pipeline (Person B): three adversarial
     perspectives run in parallel (Skeptic, Counterpart, Voss Negotiator) —
     each finds what the others would miss — then a synthesis pass merges
     them into structured CoachAnalysis, explicitly surfacing where they
...
def run_coach(state: ConversationState) -> dict:
    """Resolve the scenario, sharpen weak points, then run the debate pipeline.

    Stage 0 — scenario resolution: if a voice/typed coaching interview
    happened during prep (state.transcript) and scenario_id is "custom", an
    LLM synthesizes stakes / counterpart persona / opening line from it;
    otherwise the lp_renewal default loads. The interview is consumed here
    and the transcript cleared so rehearsal starts clean.

    Stage 1 — weak points are sharpened with a single grounded pass (using
    the interview when available), then the multi-perspective debate
    (Skeptic, Counterpart, Voss Negotiator + synthesis) enriches prep with
    structured CoachAnalysis that the proactive Wingman reads later.
    """
