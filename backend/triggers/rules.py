...
def evaluate_latest_turn(state: ConversationState) -> Nudge | None:
    ...
    if any(phrase in normalized for phrase in CONCESSION_PHRASES):
        if _is_rate_limited(state, "concession", turn_index):
            return None
        return _nudge("concession", "You may have made a commitment. Qualify it before moving on.", turn_index)
    if len(text.split()) >= MIN_MONOLOGUE_WORDS:
        if _is_rate_limited(state, "long_monologue", turn_index):
            return None
        return _nudge("long_monologue", "Pause and invite their reaction. Do not add another proof point yet.", turn_index)

    prior_user_turns = [item["text"].strip().lower() for item in transcript[:-1] if item["speaker"] == "user"]
    if normalized and normalized in prior_user_turns and not _is_rate_limited(state, "repetition", turn_index):
        return _nudge("repetition", "You are repeating a point. Ask what evidence would change their view.", turn_index)
    return None
